...
/**
 * NMS Tab Implementation for Spigot v1_19_R1
 */
public final class v1_19_R1_Tab extends TabAdapter {
    ...
    private EntityPlayer getEntityPlayer(final GameProfile profile) {
        ...
        return new EntityPlayer(server, worldServer, profile, null);
    }
    ...
    public TabAdapter hidePlayer(final Player player, final Player target) {
        if (player.canSee(target) || target.equals(player)) {
            final EntityPlayer entity = ((CraftPlayer) target).getHandle();

            sendInfoPacket(player, PacketPlayOutPlayerInfo.EnumPlayerInfoAction.e, entity);
        }
