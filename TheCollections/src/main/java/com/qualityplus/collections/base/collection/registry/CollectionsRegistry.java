...
public final class CollectionsRegistry {
    ...
    private static final Map<XMaterial, String> MATERIAL_COLLECTIONS_REGISTRY = new HashMap<>();
    ...
    public static void registerNewCollection(@NotNull final Collection collection) {
        ...
        if(collection.getCollectionExecutor().getExecutorType().equals(ExecutorType.BY_PICK_UP_MATERIAL))
            MATERIAL_COLLECTIONS_REGISTRY.put(collection.getCollectionExecutor().getMaterial(), collection.getId());
        else
    ...
    public static Optional<Collection> getByMaterial(XMaterial toMatch) {
        XMaterial key = MATERIAL_COLLECTIONS_REGISTRY.keySet().stream().filter(material -> material.equals(toMatch)).findFirst().orElse(null);

        Collection collection = key == null ? null : getByID(MATERIAL_COLLECTIONS_REGISTRY.get(key));

        return Optional.ofNullable(collection);
    }

    public static Optional<Collection> getByItem(ItemStack toMatch) {
        ItemStack key = ITEM_COLLECTIONS_REGISTRY.keySet().stream().filter(item -> item.equals(toMatch)).findFirst().orElse(null);
    ...
    public static void reloadCollections(@Inject Box box){
        ...
        MATERIAL_COLLECTIONS_REGISTRY.clear();
