...
public final class WorldEdit7 implements PasterAddon {
    ...
    public CompletableFuture<PasterSession> pasteSchematic(Location location, Schematic schematic) {
        ...
        Runnable pasteTask = () -> {
            ...
            try {
                ...
                BukkitPlayer bukkitPlayer = new BukkitPlayer(Bukkit.getOnlinePlayers().stream().findFirst().orElse(null));
                ...
                future.complete(new WorldEditSession7(editSession, getCuboid(location, clipboard, session, bukkitPlayer)));
    ...
    private Cuboid getCuboid(Location baseLoc, Clipboard cc, LocalSession session, Actor actor) {
        try {
            org.bukkit.World world = baseLoc.getWorld();
            BlockVector3 to = session.getPlacementPosition((Player) actor);
