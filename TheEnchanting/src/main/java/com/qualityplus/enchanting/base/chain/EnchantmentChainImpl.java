...
public final class EnchantmentChainImpl implements EnchantmentChain {
    ...
    private List<String> getNoShrinkLore(Map<ICoreEnchantment, Integer> enchantments){
        ...
        enchantments.forEach((ench, level) -> lore.addAll(
                StringUtils.processMulti(config.enchantmentsDisplay.enchantmentFormat, EnchantingPlaceholderUtils.getEnchantBuilder(ench)
                        .with(new Placeholder("enchanting_enchant_level_roman", MathUtils.toRoman(level)))
                        .with(new Placeholder("enchanting_enchant_level_number", level))
                        .get())
        ));
    ...
    private List<String> getShrinkLore(Map<ICoreEnchantment, Integer> enchantments){
        ...
        List<String> names = enchantments
                .entrySet()
                .stream()
                .map(entry -> StringUtils.processMulti(config.enchantmentsDisplay.enchantmentShrinkFormat, EnchantingPlaceholderUtils.getEnchantBuilder(entry.getKey())
