...
public final class BankInterfaceGUI extends BankGUI {
    ...
    private List<String> getLatestTransactions(BankData data){
        ...
        for(BankTransaction trx : data.getTransactionList()){
            ...
            String message = String.valueOf(trx.getType().equals(TransactionType.DEPOSIT) ? messages.transactionDepositAgoTime : messages.transactionWithdrawAgoTime)
                            .replace("%transaction_amount%", String.valueOf(trx.getAmount()));

            transactions.add(getParsedTime(time, message));
